...
export class AlchemyMultichainClient {
   ...
   */
  constructor(settings: AlchemyMultichainSettings, overrides?: Partial<Record<Network, AlchemyMultichainSettings>>) {
  ...
  private loadInstance(network: Network): Alchemy {
    let alchemyInstance = this.instances.get(network);
    if (!alchemyInstance) {
      ...
      alchemyInstance = new Alchemy(alchemySettings);
      this.instances.set(network, alchemyInstance);
    }
