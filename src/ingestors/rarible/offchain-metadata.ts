...
export const raribleCreatorProfileDataGetter = async (ownerAddress: string): Promise<ProfileData> => {
  const START_IDX = 0;
  ...
  const pImageUrl =
    profileData[START_IDX].imageMedia.length === 0 ? '' : profileData[START_IDX].imageMedia[START_IDX].url;
